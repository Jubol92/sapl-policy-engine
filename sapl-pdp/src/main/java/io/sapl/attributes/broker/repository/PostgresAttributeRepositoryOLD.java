...
@Slf4j
public class PostgresAttributeRepositoryOLD implements AttributeRepository {
    ...
    private final ReentrantLock                                                       lock           = new ReentrantLock(
            true);
    private final Map<RepositoryKey, PostgresAttributeRepositoryOLD.Entry>            entries        = new HashMap<>();
    private final Map<RepositoryKey, Set<PostgresAttributeRepositoryOLD.KeyObserver>> observersByKey = new HashMap<>();

    private boolean closed = false;

    @Override
    public void close() {
        Collection<PostgresAttributeRepositoryOLD.Entry> toCancel;
        lock.lock();
    ...
    public PostgresAttributeRepositoryOLD(DatabaseClient client, ObjectMapper mapper) {
        this.client = client;
        this.mapper = mapper;
    ...
    private void publishInternal(RepositoryKey key, Value value, @Nullable Duration ttl) {
        List<PostgresAttributeRepositoryOLD.KeyObserver> toFire;
        lock.lock();
        ...
        try {
            ...
            val entry = new PostgresAttributeRepositoryOLD.Entry(value);
    ...
    private void expireKey(RepositoryKey key, PostgresAttributeRepositoryOLD.Entry expectedEntry) {
        List<PostgresAttributeRepositoryOLD.KeyObserver> toFire;
        lock.lock();
    ...
    public Registration observe(@NonNull AttributeFinderInvocation invocation, @NonNull Consumer<Value> onValue) {
        ...
        val   observer = new PostgresAttributeRepositoryOLD.KeyObserver(repoKey, onValue);
        Value initial;
        lock.lock();
    ...
    private List<PostgresAttributeRepositoryOLD.KeyObserver> observers(RepositoryKey repositoryKey) {
        val bucket = observersByKey.get(repositoryKey);
        return bucket == null ? List.of() : new ArrayList<>(bucket);
    }

    private void fireObservers(List<PostgresAttributeRepositoryOLD.KeyObserver> observers, Value value) {
